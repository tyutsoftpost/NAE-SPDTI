...
class Trainer(object):
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            """
            这里我设置了1使得每次测试不在输出当前最好的epoch，而是每个epoch都输出方便画图，把1去掉就好了
            """
    ...
    def train_epoch(self):
        ...
        for i, (v_d, v_p, labels, t) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            v_d, v_p, labels = v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device)
            self.optim.zero_grad()
            v_d, v_p, f, score = self.model(v_d, v_p, t)
            if self.n_class == 1:
